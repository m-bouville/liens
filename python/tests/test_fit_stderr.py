...
fits.py had TWO independent copies of the block and only one had ever been
...
def test_a_near_singular_fit_emits_no_warning():
    ...
    with warnings.catch_warnings():
        ...
        stderr = fits._stderr_from_normal_equations(
...
def test_undefined_errors_are_reported_not_silent(capsys):
    ...
    """
    fits._stderr_from_normal_equations(_near_singular_normal_equations(), 1.0, 2)
...
def test_a_well_conditioned_fit_gives_finite_errors_and_says_nothing(capsys):
    ...
    stderr = fits._stderr_from_normal_equations(XtWX, 2.0, 2)
...
def test_an_exactly_singular_matrix_does_not_raise():
    ...
    XtWX = np.zeros((3, 3))
    XtWX[0, 0] = 1.0
    stderr = fits._stderr_from_normal_equations(XtWX, 1.0, 3)
...
def test_BOTH_fits_use_the_shared_helper():
    ...
    """
    src = source_without_comments(fits)
...
def test_the_helper_is_reachable_from_the_public_fits():
    for name in ("fit_taylor_residual_coefficients",):
        assert hasattr(fits, name)
        assert "_stderr_from_normal_equations" in source_without_comments(fits)
