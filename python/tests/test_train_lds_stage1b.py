...
def test_epochs_zero_writes_a_checkpoint_at_the_DEFAULT_ema_warmup_epochs(tmp_path, isolated_project_root):
    """
    REGRESSION: the test above passes ema_warmup_epochs=0 EXPLICITLY,
    so it never exercised the default (5) -- and ensure_lds_checkpoint,
    the real production caller of train_lds(epochs=0), doesn't pass it
    at all, so it gets that default. That gap let a real crash through:
    a grace period covering the single epoch-0 update() call meant NO
    checkpoint was written, and check_parameter_dependence.py died with
    FileNotFoundError on the ephemeral wrapper it had just asked for.
    See clamp_grace_epochs' own docstring.

    Deliberately does NOT pass ema_warmup_epochs, to exercise exactly
    ...
    stage1a_path = train_autoencoder(
        ...
        val_fraction=0.34, test_fraction=0.17, num_workers=0,
        min_step=0, min_stdev_phi=None, stats0_weight=0.01, stat_names=["avg_phi"],
        checkpoint_path=tmp_path / "stage1a_default.pt", device="cpu", seed=0,
        log_every_epoch=False, loss_curve_path=tmp_path / "curve1a_default.png",
    )
    stage2_path = train_stage2(
        base_path=base_path, resume_from=stage1a_path,
        epochs=1, batch_size=4, num_workers=0, augment=False,
        val_fraction=0.34, test_fraction=0.17, min_step=0, min_stdev_phi=None,
        checkpoint_path=tmp_path / "stage2_default.pt", device="cpu", seed=0,
        log_every_epoch=False, loss_curve_path=tmp_path / "curve2_default.png",
    )

    checkpoint_path = tmp_path / "stage3_ablation_default.pt"
    result = train_lds(
        size=32, base_path=base_path, ae_checkpoint_path=stage2_path, ae_stats_weight=0.01,
        epochs=0, batch_size=4, hidden_dim=8, n_hidden_layers=1,
        val_fraction=0.34, test_fraction=0.17, num_workers=0,
        n_rollout_steps=1, min_step=0, min_stdev_phi=None,
        encode_batch_size=4,   # NOTE: ema_warmup_epochs deliberately NOT passed
        checkpoint_path=checkpoint_path, device="cpu", seed=0,
        log_every_epoch=True, loss_curve_path=tmp_path / "curve3_ablation_default.png",
    )
    assert result == checkpoint_path
    assert checkpoint_path.exists(), (
        "epochs=0 must write a checkpoint even at the DEFAULT ema_warmup_epochs -- an "
        "unclamped grace period would swallow the single epoch-0 update() call and leave "
        "no file at all"
    )
    saved = torch.load(checkpoint_path, map_location="cpu", weights_only=True)
    assert saved["epoch"] == 0
